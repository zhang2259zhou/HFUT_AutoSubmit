...
def main():
    ...
    if stu.daily_checkin(env_dist['address']):
        requests.post('https://sc.ftqq.com/'+env_dist['sckey']+'.send?title=自动打卡成功'+output_data)
    else:
       requests.post('https://sc.ftqq.com/'+env_dist['sckey']+'.send?title=自动打开失败'+output_data)
